...
fun WordGrid(words: List<String>, hidden: Boolean, modifier: Modifier = Modifier) {
    ...
    // A plain Layout, not BoxWithConstraints, so dialogs that ask for intrinsics can host it.
    Layout(
        modifier = modifier,
        content = {
        ...
        val width = if (constraints.hasBoundedWidth) constraints.maxWidth else (longestWord + chromePx) * 2 + gapPx
        val columns = wordGridColumns(width, gapPx, chromePx, longestWord)
