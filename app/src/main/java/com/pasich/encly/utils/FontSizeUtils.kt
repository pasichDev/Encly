...
object FontSizeUtils {
     ...
     */
    fun getTextBlockFontSize(baseFontSize: Int): TextUnit = baseFontSize.coerceIn(MIN_FONT_SIZE, MAX_FONT_SIZE).sp
    ...
    fun getHeaderFontSize(baseFontSize: Int, headerType: BlockType): TextUnit {
        ...
        return when (headerType) {
            BlockType.H1 -> (baseSize + 10).sp

            // +12sp for H1
            BlockType.H2 -> (baseSize + 8).sp

            // +8sp for H2
            BlockType.H3 -> (baseSize + 6).sp

            // +6sp for H3
            BlockType.H4 -> (baseSize + 4).sp
     ...
     */
    fun getListFontSize(baseFontSize: Int): TextUnit = baseFontSize.coerceIn(MIN_FONT_SIZE, MAX_FONT_SIZE).sp
